...
router.post("/api/users/:phone", async (req, res) => {
  ...
  const { firstName, lastName, email, addPhone, address, zipCode, city } =
    req.body;
...
router.post("/api/users-fill/:email", async (req, res) => {
  ...
  const { firstName, lastName, phone, addPhone, address, zipCode, city } =
    req.body;
...
router.post("/send-sms", async (req, res) => {
  ...
  try {
    ...
    if (user?.status === "deleted") {
      return res
        .status(500)
        .json({ error: "You have deleted your account, can't login" });
    }
    if (!user) {
...
router.post("/sms-login", async (req, res) => {
  try {
    ...
    user.otp = "";
    user.otpExpiry = "";
    (user.token = deviceToken), await user.save();
    return res.status(201).json({ user, token });
  } catch (error) {
    res.status(500).json({ error: "Server Error" });
  }
  ...
  if (req.user.phone === req.body.phone) {
    try {
      ...
      if (user?.status !== "active") {
        return res
          .status(403)
          .json({ error: "No Access to perform this action" });
      }
...
router.delete(
  "/users/delete-address/:phone/:index",
  verify,
  async (req, res) => {
...
router.get("/notifications/:phone", verify, async (req, res) => {
  ...
  if (req.user.phone === req.params.phone) {
    try {
      ...
      const notifications = user.notification.slice(
        skip,
...
router.put(
  "/notifications-status/:phone/:notificationId",
  verify,
  async (req, res) => {
    if (req.user.phone === req.params.phone) {
      try {
        ...
        if (!updatedUser) {
          return res
            .status(404)
            .json({ error: "User or Notification not found" });
        }
