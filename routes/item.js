...
router.post('/create-productss', adminVerify, upload2.single('productImage'), async (req, res) => {
    try {
...
router.get('/get-products-slug/:categorySlug/:brandName', async (req, res) => {
    try {
        ...
        if (!categorySlug || !brandName) {
            return res.status(400).json({ error: 'Both categorySlug and brandName are required parameters' });
        }
...
router.get('/generate-excel/:categoryType', adminVerify, async (req, res) => {
    try {
        ...
        const response = await axios.get(`http://localhost:5000/api/category/fetch-category-name/${categoryType}`);
